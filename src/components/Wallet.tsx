...
export default function Wallet() {
    ...
    async function login() {
        ...
        // If the SDK is setup but the provider is not set, start the timer to setup a smart account
        if (!sdkRef.current.provider) {
    ...
    return (
        <Fragment>
            ...
            <div className="m-auto flex h-screen flex-col items-center justify-center gap-10 bg-gray-950">
                <h1 className="text-4x1 text-gray-50 font-bold tracking-tight lg:text-5x1">
                ...
                {!smartAccount && !loading && (
                    <button
                        onClick={login}
                        className="mt-10 rounded-lg bg-gradient-to-r from-green-400 to-blue-500 px-4 py-2 font-medium trasition-colors hover:from-green-500 hover:top-blue-600"
                    >
                        Login
                    </button>
                )}
